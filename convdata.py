...
# note: LabeledDataProvider is used instead of LabeledMemoryDataProvider to solve out of memory issue
# in LabeledMemoryDataProvider, all batches are loaded in one time
class CroppedImageNetDataProvider(LabeledDataProvider):
    # change by qiang
    # for cifar, please change it back to 32
    ...
    def __init__(self, data_dir, batch_range=None, init_epoch=1, init_batchnum=None, dp_params=None, test=False):
        ...
        # self.data_dic is empty (noneType)
        ...
        # self.cropped_data = [n.zeros((self.get_data_dims(), self.data_dic[0]['data'].shape[1]*self.data_mult), dtype=n.single) for x in xrange(2)]
        self.cropped_data = [n.zeros((self.get_data_dims(), self.batch_meta['batch_size']*self.data_mult), dtype=n.single) for x in xrange(2)]
    ...
    def get_next_batch(self):
        ...
        # we need to trim the data due to the varying size of data
        #datadic['data']=datadic['data'][:,:self.batch_meta['batch_size']]
        #datadic['labels']=datadic['labels'][:self.batch_meta['batch_size']]
        # convert to numpy array
        datadic['labels']=n.c_[n.require(datadic['labels'], dtype=n.single)]
        ...
        print 'load', datadic['labels'].shape[1], 'images from batch'
    ...
    def __trim_borders(self, x, target):
        # we need to change the order of the dimension
        # y = x.reshape(3, self.img_size, self.img_size, x.shape[1])
        y = n.transpose(x.reshape(self.img_size, self.img_size, 3, x.shape[1]),(2,0,1,3))
        # save the results
        #print 'cropped image saved'
        #for i in range(y.shape[3]):
        #    cv2.imwrite('batch_{0}.png'.format(i), n.transpose(n.array(y[:,:,:,i],'uint8'),(1,2,0)))
